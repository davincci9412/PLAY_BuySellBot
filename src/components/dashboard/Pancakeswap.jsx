...
class SaveButton extends React.Component {
    ...
    if (Number(this.props.newData.repeated_number) === 0 || this.props.newData.repeated_number === null) {
      ...
      axios.get(process.env.REACT_APP_SERVER_URL+'/address/pancakeSave', { params : {id: this.props.newData.id, repeated_number: this.props.newData.repeated_number, slippage: this.props.newData.slippage}}).then( async res => {
        const result = await res.data   
        if (result !== "fail"){
          this.setState({repeated_number: result.pancakeRepeatedNumber});  
          this.setState({slippage: result.pancakeSlippage});  
          this.setState({id: result._id});   
          this.setState({start: result.pancakeActionStatus});  
...
class Pancakeswap extends React.Component {
  ...
  onStart( data ){
    ...
    axios.get(process.env.REACT_APP_SERVER_URL+'/address/pancakeStart', { params : {id: data.id, repeated_number: data.repeated_number, slippage: data.slippage}}).then( async res => {
      const result = await res.data   
      if (result !== "fail"){
  ...
  onStop( data ){
    //Make the start status of trading to the "settings" table of DB
    axios.get(process.env.REACT_APP_SERVER_URL+'/address/pancakeStop', { params : {id: data.id, repeated_number: data.repeated_number, slippage: data.slippage}}).then( async res => {
      const result = await res.data   
      if (result !== "fail"){
  ...
  render(){
	  return <div className="exchange wallet">
				<div className="container-fluid small" datas={this.state.datas} >
					<div className="mb-5">
            <div >
              <h1 className="text-center text-success">Pancakeswap Setting Status</h1>
              ...
              <h1 className="text-center text-success mt-5">Pancakeswap Action Control</h1>
              <div className='search-box'>
                <div className='container-fluid'>
                  <div className="row">
