...
class Input extends React.Component {
  ...
  render(){
	  return <input type='text' name={this.props.name} onChange={this.props.change} onClick={this.props.click} autoComplete="off" value={this.props.value} className='form-control'/>
  }
}
class Select extends React.Component {
  ...
  render(){
	return <select type='text' name={this.props.name} onChange={this.props.change} value={this.props.value} className='form-control'>	
        ...
        <option value="1440">1440 (Every 1 Minutes)</option> 
      </select>
  }
}
...
class SaveButton extends React.Component {
  ...
  onClick(){    
    ...
    axios.get(process.env.REACT_APP_SERVER_URL+'/address/saveSetting', { params : {id: this.props.newData.id, trade_amount: this.props.newData.trade_amount, repeated_number: this.props.newData.repeated_number, slippage: this.props.newData.slippage}}).then( async res => {
      const result = await res.data   
      if (result !== "fail"){
        this.setState({trade_amount: result.tradeAmount});
        this.setState({repeated_number: result.repeatedNumber});  
        this.setState({slippage: result.slippage});  
        this.setState({id: result._id});   
        this.setState({start: result.actionStatus});  
...
class StartButton extends React.Component {
  ...
  onClick(){    
    if (Number(this.props.newData.trade_amount) === 0 || this.props.newData.trade_amount === "") {
      toast.error("Input the trading amount.", {position: toast.POSITION.TOP_RIGHT,  autoClose:3000});
    } else if (Number(this.props.newData.repeated_number) === 0 || this.props.newData.repeated_number === ""){
...
class Wallet extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = { datas: datasByRow, addressValid: false,  chainValid: false,  formValid: false, id:"", slippage:"", trade_amount:"", repeated_number:"", start:""};
    this.onStart = this.onStart.bind(this);
    this.onStop = this.onStop.bind(this);
    this.onSave = this.onSave.bind(this);

    //Get the setting information from DB
    axios.get(process.env.REACT_APP_SERVER_URL+'/address/getSetting').then( async res => {
      const result = await res.data   
      if (result !== null){
        this.setState({trade_amount: result.tradeAmount});
        this.setState({repeated_number: result.repeatedNumber});  
  ...
  onStart( data ){
    ...
    axios.get(process.env.REACT_APP_SERVER_URL+'/address/start', { params : {id: data.id, trade_amount: data.trade_amount, repeated_number: data.repeated_number, slippage: data.slippage}}).then( async res => {
      const result = await res.data   
      if (result !== "fail"){
  ...
  onStop( data ){
    //Make the start status of trading to the "settings" table of DB
    axios.get(process.env.REACT_APP_SERVER_URL+'/address/stop', { params : {id: data.id, trade_amount: data.trade_amount, repeated_number: data.repeated_number, slippage: data.slippage}}).then( async res => {
      const result = await res.data   
      if (result !== "fail"){
  ...
  render(){
	  return <div className="exchange wallet">
				<div className="container-fluid small" datas={this.state.datas} >
					<div className="mb-5">
            <div >
              <h1 className="text-center text-success">Setting Status</h1>
              <div className='search-box'>
                <div className='container-fluid'>
                  <div className="row my-3">
                    <div className = 'col-md-8'>
                      <h4>Trading amount($) : </h4>
                    </div>          
                    <div className='col-md-4'>
                      <Input name='trade_amount' change={this.handleChange.bind(this, 'trade_amount')} value={this.state.trade_amount} />          
                    </div>
                  </div>
                  ...
                  <div className="row my-4">
                    <div className="col-md-12">
                      <h6 className ="text-danger">* If you change the values, don't forget to save the values before you start the trading.</h6>
                    </div>
                  </div>
              ...
              <h1 className="text-center text-success mt-5">Action Status</h1>
              <div className='search-box'>
                <div className='container-fluid'>
                  <div className="row">
